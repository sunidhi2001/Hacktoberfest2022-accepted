...
def intro():
    ...
    print(
        magenta("SNAKE (🐍) WATER (💧) GUN (🔫)".center(columns))
    )  # Printing the name of the game

    sleep(1.5)  # Making program to sleep to print next statement after sometime

    print(
        cyan("GAME DEVELOPED BY - 👑 KRISH JAIN" "" " 👑".center(columns))
    )  # Printing the name of the developer

    sleep(
        2
    )  # Making program to sleep to execute next part of the program after sometime
...
def choices():
    ...
    options = [
        ...
        "Water",
        "Gun",
    ]  # List of options for computer to choose randomly
    computer_choice = random.choice(
        options
    )  # function to store random choice from list 'options'
...
def results():
    ...
    if computer_choice == "Snake":
        if user_choice == "s" or user_choice == "snake":
            ...
            )  # Error message in 'HINDI' for invalid choice
...
while 1:  # Infinite loop to play the game as many times as the user wants
    computer_points, user_points, flag, chance = (
        0,
        0,
        0,
        0,
    )  # Initialising values to zero at the beginning of the game
    computer_choice, user_choice, replay = (
        "",
        "",
        "",
    )  # Emptying strings at the beginning of the game
    ...
    while chance < n:  # Iterating loop 'n' times for playing 'n' number of chances
        ...
        if (
            flag == 0
        ):  # Incrementing flag's value only if the input given by the user will be valid
            chance += 1
